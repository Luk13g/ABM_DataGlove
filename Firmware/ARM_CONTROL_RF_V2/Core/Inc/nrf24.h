...
#define nRF24_CMD_ACTIVATE         (uint8_t)0x50 // Lock/unlock exclusive features
#define nRF24_CMD_R_RX_PL_WID      (uint8_t)0x60 // Ack payload width
#define nRF24_CMD_W_ACK_PAYLOAD    (uint8_t)0xA8 // Ack payload register
#define nRF24_CMD_W_TX_PLD_NO_ACK  (uint8_t)0xB0 // TX without automatic ACK
...
#define nRF24_MASK_FEATURE_REG     (uint8_t)0x07 // Feature register mask
#define nRF24_MASK_EN_DPL          (uint8_t)0x04 // Enables Dynamic Payload Length
#define nRF24_MASK_EN_ACK_PAY      (uint8_t)0x02 // Enables Payload with ACK
#define nRF24_MASK_EN_DYN_ACK      (uint8_t)0x01 // Enables the W_TX_PAYLOAD_NOACK command
...
typedef enum nRF24_pipe_number_e {
	...
	nRF24_PIPE_INVALID = (uint8_t)0xFF
} nRF24_pipe_number_t;
...
nRF24_RXFIFO nRF24_ReadRxFifoPayload(uint8_t *pBuf, uint8_t length);
